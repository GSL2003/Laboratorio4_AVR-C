...
//HARDWARE: ATMEGA328P
//CREADO: 12/04/2024
//ÚLTIMA MODIFICACIÓN: 15/04/2024 - 05:05
//******************************************************************************************************************
...
int main(void)
{
	...
	//-> Se llaman a las funciones para integrarse a la función principal --- LOOP
	ADC_IN();
	setup();
	b_inter();
	...
	while(1)
	{
		...
		PORTD = DISP_HEX[LOWB_4];				//Valor de las unidades 
		...
		dec_hex(ADC_contador);
		PORTD = DISP_HEX[HIGHB_4];				//Valor de las decenas

		//Configuración de los transistores
		PORTC |= (1<<PC3);					//Se enciende el transistor del segundo Display
		PORTC &= ~(1<<PC4);					//Se apaga el transistor del primer Display
		_delay_ms(DL_MX);
		
		//Configuración de la Alarma//
		if (ADC_contador > Bi_contador){
			PORTC |= (1<<PC5);					//Si el valor del ADC es mayor al contador binario, se enciende la LED
			} else {
			PORTC &= ~(1<<PC5);					//Si el valor del ADC no es mayor al contador binario, la led se mantiene apagada
		}
...
void setup(void){
	...
	// - Pines del puerto D - Display de 7 segmentos
	DDRD = 0xFF;
	PORTD = 0;
	...
	// PC2 - Led contador 8 bits --- PC3 y PC4 transistores para display de 7 segmentos ---- PC5 LED-Alarma
...
ISR(PCINT1_vect){
	...
	Boton2_S2 = PINC&(1<<PINC1);				//Se configura la entrada del boton PC1
